...
def comprobar_importe(valor: str) -> bool:
    ...
    if valor.startswith("-"):
        numero_negativo = valor[1:]
        
        for i in numero_negativo:
                
            if i == ".":
                numero_negativo.split(".")
                numero_negativo = numero_negativo[0] + numero_negativo[1]
                
        valor = numero_negativo
    else:
        for i in valor:
...
def comprobar_comando(comando: str) -> bool:
    ...
    """
    comprobacion_comando = True
    operacion_y_cantidad = comando.split(" ")
    operacion = operacion_y_cantidad[0]
    
    for operacion in COMANDOS:
        if operacion not in comando:
            comprobar_comando = False
